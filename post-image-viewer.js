...
WPIV = {
	...
	'show'	: function() {
		...
		$imgEl = WPIV._elImgWrp.find('img:first');
		$ImgElCurrent = WPIV._elImgWrp.find('img:last');		
		...
		//appear as a cross-fading slideshow
		var imgLoad = imagesLoaded( $imgEl );
		...
		imgLoad.on( 'done', function( instance ) {
			...
			WPIV._elImgWrp.find('img:first').fadeTo(parseInt(WPIV.cfg.fadeDuration),1,'linear');
			WPIV._elImgWrp.find('img:last').fadeTo(parseInt(WPIV.cfg.fadeDuration),0, 'linear',function() {
					WPIV._busyLoadingImg = 0;
			});
	...
	'showarrows' : function (showit) {
		...
		//no navigation if one image in array
