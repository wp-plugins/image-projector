...
function wpiv_load_script()
{
	...
	$galleryident	= $options['gallery_identifier'];
...
function wpiv_load_external_scripts() {
	...
	wp_register_style( 'wpiv_gfont', "https://fonts.googleapis.com/icon?family=Material+Icons",array(),THIS_VERSION);
	wp_enqueue_style( 'wpiv_gfont');
...
function wpiv_generate_postimage_reference_js() {
	...
	if ($pageposts):
		...
		foreach ($pageposts as $post):
			...
			$foundimg = wpiv_get_all_post_images($post->ID);
			if(!empty($foundimg)) {
...
function wpiv_get_all_post_images($postid) {
	global $wpdb;
	...
	foreach ($matches[1] as $match)
	{
		...
		if ( isset($match[$inum]) ) $img = $match;

		$foundimg[$inum]['name'] 	= trim($img);
		$foundimg[$inum]['width'] = 0;
		$foundimg[$inum]['height'] = 0;
...
if(!isset($this->options['gallery_identifier'])) $this->options['gallery_identifier'] = 'gallery, envira-gallery-wrap, ngg-galleryoverview';
    ...
    {   
		...
		add_settings_field(
            'gallery_identifier', 'Gallery container identifier(s)',  array( $this, 'gallery_identifier_callback' ), 
            'wpiv-setting-admin', 'wpiv_settings_main'
        );  
		add_settings_field(
    ...
    public function sanitize( $input )
    {
        ...
        if( isset( $input['gallery_identifier'] ) )
            $new_input['gallery_identifier'] = sanitize_text_field( $input['gallery_identifier'] );
		...
		$new_input['expected_loadtime'] = sanitize_text_field( $input['expected_loadtime'] );
		 if((int)$new_input['expected_loadtime'] < 1) {
		...
		if( isset( $input['menu_height'] ) ){
			$new_input['menu_height'] = sanitize_text_field( $input['menu_height'] );
			 if((int)$new_input['menu_height'] < 30 || (int)$new_input['menu_height'] > 200) {
    ...
    public function print_main_section_info()
    {
        ...
        print  "<hr>Unless specified below, all image elements will open in this plugin. You may limit the images by  giving one or more element ids and/or class names, separated by comma. Example: <i>#postimage, .masonryimage</i>";
    }
    ...
    {   
        printf(
            '<input type="text" style="width:400px" id="identifier" name="wpiv_options[identifier]" value="%s" >&nbsp;classes or IDs separated by comma</input>',
            isset( $this->options['identifier'] ) ? esc_attr( $this->options['identifier']) : ''
        );

    }
    public function gallery_identifier_callback()
    {   
        printf(
            '<input type="text" style="width:400px" id="gallery_identifier" name="wpiv_options[gallery_identifier]" value="%s">&nbsp;classes or IDs separated by comma</input>',
            isset( $this->options['gallery_identifier'] ) ? esc_attr( $this->options['gallery_identifier']) : ''
        );
	...
	if( ! $opt)
		{
			...
			$opt['captions'] = '1';	
			//VERSION 1.6
			$opt['gallery_identifier'] = 'gallery, envira-gallery-wrap, ngg-galleryoverview';
