...
function wpiv_load_script()
{
		...
		|| (is_archive() && $add_archive == '1')))
	{
		...
		$localfile 			= THIS_PLUGIN_DIR."/ref/wpiv_ref.js";
		$localfilecaption 	= THIS_PLUGIN_DIR."/ref/wpiv_ref_c.json";
	...
	wp_register_script( 'wpiv_reference', THIS_PLUGIN_URL.'/ref/wpiv_ref.js',array(),THIS_VERSION);
...
function wpiv_generate_postimage_reference_js() {
	...
	$jsFile = $refDir."/wpiv_ref.js";
	$handle = fopen($jsFile,"w");
	//caption
	if($doCaption) {
		$jsonFile =  $refDir."/wpiv_ref_c.json";
	...
	if ($pageposts):
		...
		foreach ($pageposts as $post):
			...
			$localfile = str_replace(THIS_UPLOAD_URL,"",$pinfo[name]);
			...
			if($doCaption) {
				...
				if($counts<$numrows) fwrite($jsonHandle, ',');
			}
			//end caption
		...
		//end caption		
		fwrite($handle, '];');
		fwrite($handle, 'var wpiv_slidesequence = [];wpiv_slidesequence=[0,');
		$i=1;
